...
int main(int argc, char **argv) {
...

    {
        ...
        catch (const sepolia::is_not_allocated &a) {
            fc();
            cout << (a.what() == E0001) << endl;
        }
        try {
            vector_dense<double> vd2(TEST_DIM, TEST_VALUE1);
            vd2.get_element(TEST_DIM);
        }
        catch (const sepolia::index_out_of_range &a) {
            fc();
            cout << (a.what() == E0002) << endl;
        }
    }
